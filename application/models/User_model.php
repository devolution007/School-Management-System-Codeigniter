...
class User_model extends MY_Model
{
    ...
    public function checkScretchPin($spin)
    {
        $data = $this->db->where("code",$spin)->get("scretch_card_pins")->row();
        return $data;
    }
    public function ExpireScretchPin($data)
    {
        $limits = $data->limits - 1;
        $data = $this->db->where("code",$data->code)
        ->update("scretch_card_pins",array("limits" => $limits));
        return $data;
    }
