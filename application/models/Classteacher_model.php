...
class Classteacher_model extends MY_Model
{
    ...
    public function AddSubjectTeacher($data){
        return $this->db->insert('subject_teacher',$data);
    }
public function teacherBysubject()
    {
        $query = $this->db->select('staff.*,subject_teacher.id as ctid,subject_teacher.subject_id')->join("staff", "subject_teacher.staff_id = staff.id")->get("subject_teacher");
        return $query->result_array();
    }
