...
function calculate_grade($total_achieved, $term)
{
    $CI = &get_instance();
    $grade_settings = $CI->grade_setting_model->get_grade_settings($term);
    $grade = 'F'; // Default grade

    foreach ($grade_settings as $setting) {
        if ($total_achieved >= $setting['min_score'] && $total_achieved <= $setting['max_score']) {
            $grade = $setting['grade'];
            break; // Found the matching grade, exit the loop
        }
    }
...
// Within a relevant controller method
 function determineGrade($totalAchieved,$term,$class_id)
{
    $CI           = &get_instance();
    $gradeSettings = $CI->db->where('term',$term)->where('class_id',$class_id)->get('grade_settings')->result();
    $grade = 'F'; // Default grade
    foreach ($gradeSettings as $setting) {
        if ($totalAchieved >= $setting->min_score && $totalAchieved <= $setting->max_score) {
            $grade = $setting->grade;
            break;
        }
    }
...
function maxScore($grade,$term)
{
    $CI           = &get_instance();
    $gradeSettings = $CI->db->where('term',$term)->where('grade',$grade)->get('grade_settings')->row();
    if(!empty($gradeSettings)){
        return $gradeSettings->max_score;
    }else{
        return 0;
    }
   
}
function minScore($grade,$term)
{
    $CI           = &get_instance();
    $gradeSettings = $CI->db->where('term',$term)->where('grade',$grade)->get('grade_settings')->row();
    if(!empty($gradeSettings)){
        return $gradeSettings->min_score;
    }else{
        return 0;
    }
...
function permotion($marks)
{
    $CI           = &get_instance();
    $CI->db->where('min_score <=', $marks);
    $CI->db->where('max_score >=', $marks);
    $data = $CI->db->get('permotion_settings')->row();
    if(!empty($data)){
        return $data->message;
    }else{
        return '';
