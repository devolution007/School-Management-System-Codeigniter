...
class User extends Student_Controller
{
    ...
    public function studentbehaviour()
    {
        ...
        $this->session->set_userdata('top_menu', 'studentbehaviour');
        $student_id            = $this->customlib->getStudentSessionUserID();
      
        $this->load->view('layout/student/header');
        if($this->session->userdata('student')['role'] == 'parent'){
            $this->load->view('user/studentSearch', $data);
        }else{
               
        $this->load->view('user/studentbehaviour/index',$data);
        }
    ...
    public function searchvalidation()
    {
        ...
        $session = $this->input->post('session');
            ...
            if ($this->form_validation->run() == true) {
                ...
                $params = array('srch_type' => $srch_type, 'class_id' => $class_id, 'session' => $session);
                ...
                $error['session'] = form_error('session');
    ...
    public function dtstudentlist()
    {
        ...
        $resultlist = $this->student_model->searchdtByClassSession($class, $session,$this->session->userdata('student')['id']);
        $students = array();
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $viewbtn = "<a href='" . base_url() . "user/user/student_behaviour/" . $student->id . "' target='_blank'   class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('view') . "'><i class='fa fa-reorder'></i></a>";
    ...
    public function student_behaviour($id){
        $this->db->select("students.id,students.firstname,' ',students.lastname,student_behaviour.id as remarks_id,student_behaviour.remarks,student_behaviour.comments")->from("student_behaviour");
        ...
        $this->db->join('sections', 'sections.id = student_session.section_id');
         ...
         $this->load->view('layout/student/header', $data);
         $this->load->view('user/studentbehaviour/student_behaviour',$data);
         $this->load->view('layout/student/footer', $data);
    }
    public function add_comment_on_remarks(){
      
        $this->form_validation->set_rules('comment', 'Comments', 'trim|required|xss_clean');
        
        if ($this->form_validation->run() == true) {
            $data=array('comments'=>$this->input->post('comment'));
            $this->db->where('id',$this->input->post('remarks_id'));
            $this->db->update('student_behaviour',$data);
            $array  = array('status' => 1, 'error' => '', 'message' => 'Comments Added Successfully');
            echo json_encode($array);

        } else {
            $error             = array();
            $error['comments'] = form_error('comments');
            $array             = array('status' => 0, 'error' => $error);
            echo json_encode($array);
        }
    }
    public function get_student_behaviour(){
        ...
        if (!empty($data)) {
            foreach ($data as $student_key => $student) {
                ...
                $row[] = $student->remarks . (!empty($student->comments) ? "<br><b>Comments:</b> " . $student->comments : '');
    ...
    public function studentResults()
    {
        ...
        $this->session->set_userdata('sub_menu', 'studentResults');
    ...
    public function search()
    {
        ...
        ->where("results.class_id",$class)
        ->where("results.s_id",$session)
        ->get()->result();
    ...
    public function print_terms($id)
    {
        ...
        {
            $data["student"] = $this->db->where('id',$data["result"]->s_id)->get('students')->row_array();
