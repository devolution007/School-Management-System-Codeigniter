...
class Site extends Public_Controller
{
    ...
    public function varify_pin()
    {
        $ScretchPin_Details = $this->user_model->checkScretchPin($this->input->post("scratch_pin"));
        if(!empty($ScretchPin_Details)  && $ScretchPin_Details->u_id > 0)
        {

            $user = $this->db->select("email")->from("students")->where("id",$ScretchPin_Details->u_id)->get()->row();
            $ScretchPin_Details->email = $user->email;

        }
        echo json_encode($ScretchPin_Details);
    }

    public function verify_scratch_pins()
    {
        $this->form_validation->set_rules('serial_no', "Serial Pin", 'trim|required');
        $this->form_validation->set_rules('code', "Code", 'trim|required');
        if ($this->form_validation->run() == false) 
        {
            echo json_encode(array("status" => false,"message" => "Complete your all fields"));
            return;
        }
        else
        {
            $data = $this->db->select("*")->from("scretch_card_pins")
            ->where("serial_no",$this->input->post('serial_no'))    
            ->where("code",$this->input->post('code'))->get()
            ->row();
            if(!empty($data))
            {
               if($data->u_id == 0)
               {
                   echo json_encode(array("status" => true,"active" => false,"message" => "Pin needs Activatation "));
                   return;
                }    
               else
               {
                if($data->limits > 0){ 
                 $stcr = $this->db->select("students.id,students.email,users.password")
                    ->from("students")
                    ->join("users","students.id = users.user_id","inner")
                    ->where("students.id",$data->u_id)
                    ->limit(1)
                    ->get()->row();
                    $update_limits = $this->db->where("serial_no",$this->input->post('serial_no'))->update("scretch_card_pins",["limits" => ($data->limits-1)]);
                    echo json_encode(array("status"=>true,"active" => true,"credeintials" => $stcr,"message" => "activated")); 
                    return;
                }
                else
                {
                 echo json_encode(array("status"=>false,"message" => "Your pin limit has been reached")); 
                 return;
                }
               }
            ...
            {
                echo json_encode(array("status" => false,"message" => "serial code is wrong")); 
                return;
            }
    ...
    public function  activate_pin()
    {
        ...
        {
            $data = $this->db->select("students.id,students.email,users.password")
            ->from("students")
            ->join("users","students.id = users.user_id","inner")
            ->where("students.admission_no",$this->input->post("student_id"))
            ->limit(1)
            ->get()->row();
            
            $updateresp = $this->db->where("serial_no",$this->input->post("serial_no"))->update("scretch_card_pins",array("u_id" => $data->id));
            if($updateresp)
            {
                echo json_encode(array("status"=>$updateresp,"credeintials" => $data,"message" => "activated")); 
                return;
            }
            else
            {
                echo json_encode(array("status"=>$updateresp,"message" => "activation failed!")); 
                return;
            }
        }
    }
