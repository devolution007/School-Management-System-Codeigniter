...
class Teacher extends Admin_Controller
{
    ...
    {
        if (!$this->rbac->hasPrivilege('assign_subject_teacher', 'can_view')) {
            access_denied();
        }
       
        
        $this->session->set_userdata('top_menu', 'Academics');
        $this->session->set_userdata('sub_menu', 'admin/teacher/assign_subject_teacher');
        $data['title']      = 'Add Subject Teacher';
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $student_exam_info = $this->input->post("student_exam_info")??0;
            foreach ($subjects as $key => $value) {

                $data = array(
                    'subject_id'         => $value,
                    'staff_id'         => $teachers,
                    'student_exam_info'         => $student_exam_info,
                );
                
                $this->classteacher_model->AddSubjectTeacher($data);
            }
            $this->session->set_flashdata('msg', '<div class="alert alert-success">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/teacher/assign_subject_teacher');
        } 
        $subjectlist         = $this->subject_model->get();
        $data['subjectlist'] = $subjectlist;

        $assignteacherlist         = $this->subject_model->getSubjectTeacher();
        $data['assignteacherlist'] = $assignteacherlist;
       
        $data["tlist"] = $this->classteacher_model->teacherBysubject();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('class/subjectTeacher', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function subjectteacherdelete($id){
        if ((!empty($id)) ) {
