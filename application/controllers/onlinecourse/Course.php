...
class Course extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($course_provider == "html5") {
                if (isset($_FILES['html5'])) {
                    $name        = $_FILES['html5']['name'];
                    $file_name   = time() . "-" . uniqid(rand()) . "!" . $name;
                    $destination = "./uploads/course/video/" . $file_name;
                    if (move_uploaded_file($_FILES['html5']["tmp_name"], $destination)) {
                        $html5_video = $file_name;
                    }
