...
class Schsettings extends Admin_Controller
{
    ...
    public function ajax_editlogo()
    {
        ...
        if ($width <= $maxWidth && $height <= $maxHeight) {
            ...
            $data = array(
                'file' => "Image dimensions exceed the maximum allowed size (200x200).",
            );
            $array = array('success' => false, 'error' => $data);
            echo json_encode($array);
        }
    ...
    public function ajax_editadmin_smalllogo()
    {
        ...
        $data = array(
            'file' => "Image dimensions exceed the maximum allowed size (60 X 60).",
        );
        $array = array('success' => false, 'error' => $data);
        echo json_encode($array);
    }
    ...
    public function ajax_editadmin_adminlogo()
    {
        ...
        $data = array(
            'file' => "Image dimensions exceed the maximum allowed size (200 X 200).",
        );
        $array = array('success' => false, 'error' => $data);
        echo json_encode($array);
    }
    ...
    public function studentguardianpanel()
    {
        ...
        $setting->base_url = ($setting->base_url == "") ? base_url() : $setting->base_url;
        $setting->folder_path = ($setting->folder_path == "") ? FCPATH : $setting->folder_path;
        $data['scretch_cards'] = $this->db->get("scretch_cards")->result();
        $data['result']  = $setting;
        $this->load->view('layout/header');
    ...
    public function studentguardian()
    {
        ...
        $data = array(
            ...
            'scratch_pin_login'   =>  $this->input->post('scratch_pin_login'),
            'result_setting'   =>  $this->input->post('result_setting')
        );

        $this->setting_model->add($data);

        $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'), 'response'  => $this->input->post());
        echo json_encode($array);
    }
    public function ScretchCardSave()
    {

        if ($this->input->post("id")) {
            $this->db->where('id', $this->input->post("id"));
            $this->db->update('scretch_cards', array(
                "login_limit" => $this->input->post("login_limit"),
                "unlimited" => $this->input->post("unlimited"),
                "card_quantity" => $this->input->post("card_quantity"),
                "description" => $this->input->post("description"),
            ));
        } else {
            $this->db->insert('scretch_cards', $this->input->post());
            $id = $this->db->insert_id();
            for ($i = 0; $i < $this->input->post("card_quantity"); $i++) {
                $this->db->insert('scretch_card_pins', array(
                    "s_id" => $id,
                    "serial_no" => rand(100000000000, 999999999999),
                    "code" => $this->random_strings(8),
                    "limits" => $this->input->post("login_limit"),
                    "unlimited" => $this->input->post("unlimited") ?? false,

                ));
            }
        }
        return redirect("/schsettings/studentguardianpanel");
    }
    public function ScretchCardDlt($id)
    {
        if ($id) {
            $this->db->where('id', $id)->delete('scretch_cards');
            $this->db->where('id', $id)->delete('scretch_card_pins');
        }
        return redirect("/schsettings/studentguardianpanel");
    }

    public function viewpins($id)
    {
        $data["scretch_cards"] = $this->db->where("id", $id)->get("scretch_cards")->row_array();
        $data["scretch_cards"]["scretch_card_pins"] = $this->db->where("s_id", $id)->get("scretch_card_pins")->result();
        $this->load->view('setting/prints_pins', $data);
    }
    ...
    public function exam_settings()
    {
        ...
        $data['result']           = $setting;
        $this->load->view('layout/header', $data);
        $this->load->view('setting/exam_settings', $data);
        $this->load->view('layout/footer', $data);
    }
    public function exam_settings_Save()
    {

        $this->form_validation->set_rules('num_of_assessments', "Num Of Assessments", 'trim|required|xss_clean|min_length[1]|max_length[10]');
        if ($this->form_validation->run() == false) {
            $data = array(
                'num_of_assessments'  => form_error('num_of_assessments'),
            );
            $array = array('status' => 'fail', 'error' => $data);
            echo json_encode($array);
        } else {
            ...
            $data = array(
                ...
                'id'  => $this->input->post('sch_id'),
                'num_of_assessments' => $this->input->post('num_of_assessments'),
                "assessments_detail" => json_encode($this->input->post('assessments')),
                "assessments_marks" => json_encode($this->input->post('marks'))
    ...
    public function grade_setting()
    {
        $this->session->set_userdata('top_menu', 'System Settings');
        $this->session->set_userdata('sub_menu', 'schsettings/index');
        $this->session->set_userdata('subsub_menu', 'schsettings/grade_setting');

        $data['grade_settings'] = $this->db->get("grade_settings")->result();
        $data['classes'] = $this->db->get("classes")->result();
        $this->load->view('layout/header');
        $this->load->view('setting/grade_setting', $data);
        $this->load->view('layout/footer');
    }
    
    public function GradeSettingSave()
    {
        if (!empty($this->input->post('class_id')) && !empty($this->input->post('max_score')) && !empty($this->input->post('min_score'))&&!empty($this->input->post('term'))&&!empty($this->input->post('grade'))) {

           
          
                foreach($this->input->post('class_id') as $key=>$value){
                    $data=$this->db->where('class_id',$value)->where('term',$this->input->post('term'))->where('grade',$this->input->post('grade'))->get('grade_settings')->row();
                    if(empty($data)){ 
                    $grade_array =array(
                        'class_id' => $value,
                    ...
                    $this->db->insert('grade_settings',$grade_array);
                }
                }
               
                $array = array('success' => true, 'error' => '', 'message' => "Save Successfully");
                echo json_encode($array);           
        }else{
            $error = array(
                'grade_error'  =>  "All Field Required",
            );
    ...
    {
        if ($id) {
            $this->db->where('id', $id)->delete('grade_settings');
        }
        $array = array('success' => true, 'error' => '', 'message' => "Data Deleted Successfully");
            echo json_encode($array);
    }
    
    public function GradeSettingUpdate()
    {
       
        if (!empty($this->input->post('class_id')) && !empty($this->input->post('max_score')) && !empty($this->input->post('min_score'))&&!empty($this->input->post('term'))&&!empty($this->input->post('grade'))) {

            $this->db->where('id',$this->input->post('id'))->update('grade_settings',$this->input->post());
            $array = array('success' => true, 'error' => '', 'message' => "Update Successfully");
    ...
    public function permotion_setting()
    {
        $this->session->set_userdata('top_menu', 'System Settings');
        $this->session->set_userdata('sub_menu', 'schsettings/index');
        $this->session->set_userdata('subsub_menu', 'schsettings/permotion_setting');

        $data['permotion_settings'] = $this->db->get("permotion_settings")->result();
        $this->load->view('layout/header');
        $this->load->view('setting/permotion_setting', $data);
        $this->load->view('layout/footer');
    }
    
    public function permotion_setting_save()
    {
        if (!empty($this->input->post('max_score')) &&!empty($this->input->post('message'))) {

            $data=$this->db->where('message',$this->input->post('message'))->get('permotion_settings')->row();
            if(empty($data)){
                $this->db->insert('permotion_settings',$this->input->post());
                $array = array('success' => true, 'error' => '', 'message' => "Save Successfully");
                echo json_encode($array);
            }else{
                $error = array(
                    'grade_error'  =>  $this->input->post('message').' Already Exist.',
                );
                $array = array('status' => 'fail', 'error'=>$error);
            echo json_encode($array);
            }
    ...
    public function delete_permotion($id)
    {
        if ($id) {
            $this->db->where('id', $id)->delete('permotion_settings');
        }
        $array = array('success' => true, 'error' => '', 'message' => "Data Deleted Successfully");
            echo json_encode($array);
    }
    
    public function permotion_setting_update()
    {
       
        if (!empty($this->input->post('max_score')) && !empty($this->input->post('min_score'))&&!empty($this->input->post('message'))) {

            $this->db->where('id',$this->input->post('id'))->update('permotion_settings',$this->input->post());
            $array = array('success' => true, 'error' => '', 'message' => "Update Successfully");
            echo json_encode($array);
            redirect($_SERVER['HTTP_REFERER']);
